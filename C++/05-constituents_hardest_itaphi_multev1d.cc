...
int main()
{
  ...
  ifstream fin("Data/FASTJET-INPUT3_bck_ppjj_mistagging");
  // ofstreame opening
  ...
  int index=0, f = 0;
  string base("05-constituents_hardest_itaphi_multevd.dat");
  string loc("output/05-constituents_hardest_itaphi_multevd/");
  // string base("/mafd");
  while (fin >> fourvec[0] >> fourvec[1] >> fourvec[2] >> fourvec[3] >> pdg ) {
    //cout<<fourvec[0]<<endl;
    if(fourvec[0]==0 and fourvec[1]==0 and fourvec[2]==0 and fourvec[3]==0 and pdg==0 ){
        double Et=0; 
